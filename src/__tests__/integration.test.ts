...
import { clubsData, fansData, eventsData, ordersData } from "../db/data/index";
import app from "../app";
import { NewClub, NewEvent, NewFan, NewOrder } from "../helpers/interfaces";

beforeEach(() => {
  return seed({ clubsData, fansData, eventsData, ordersData });
});
    ...
    return request(app)
      ...
      .then((response) => {
        ...
        expect(response.body.address).toBe(
          "24, Tauren Lane, Manchester, England, M13 0FZ"
        );
  ...
  test("GET 200: returns all clubs data", () => {
    return request(app)
      ...
      .then(({ body }) => {
        ...
        body.clubs.forEach((club: NewClub) => {
  ...
  test("GET 200: returns all events data", () => {
    return request(app)
      ...
      .then(({ body }) => {
        ...
        body.events.forEach((event: NewEvent) => {
...
describe("GET /api/fans/:fanId/orders", () => {
  test("GET 200: returns all orders belonging to a fan ID", () => {
    return request(app)
      ...
      .expect(200)
      .then(({ body }) => {
        body.orders.forEach((order: NewOrder) => {
